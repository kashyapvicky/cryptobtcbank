...
$('#bitcoin_tab').click(function() {
    $('.sw-btn-finish').show();
    document.getElementById("active_tab").value = "bitcoin_tab";

});

$('#blockchain_tab').click(function() {
    $('.sw-btn-finish').show();
    $('.sw-btn-finish').prop('disabled', false);
    document.getElementById("active_tab").value = "blockchain_tab";

});

$('#bitgo_tab').click(function() {
    $('.sw-btn-finish').show();
    $('.sw-btn-finish').prop('disabled', false);
    document.getElementById("active_tab").value = "bitgo_tab";

});
$('#bank_transfer').click(function() {
    $('.sw-btn-finish').show();
    $('.sw-btn-finish').prop('disabled', true);
    document.getElementById("active_tab").value = "bank_transfer";
});
...
function addNewraw()
{
    ...
    $('<tr id = "epin_raw' + j + '" ><td class="hidden-xs center">' + j + '</td> <td  class="center"><p><label for="p_scnts" style="display: none;"><div class="col-md-12"><input type="text" id="epin' + j + '" size="13" name="epin' + j + '" value="" placeholder="PIN"  autocomplete="off" class="form-control" onblur="check_epin_submit();"/></div><span id ="pin_box_' + j + '"> </span></label></td><td class="hidden-xs center"><div class="col-md-12"><input type="text" id="pin_amount' + j + '" size="13" readonly="true" class="form-control"/></div></td><td class="hidden-xs center"><div class="col-md-12"><input type="text" id="remaining_amount' + j + '" size="13" readonly="true" class="form-control"/></div></td><div class="col-md-12"><td class="hidden-xs center"><div class="col-md-12"><input type="text" id="balance_amount' + j + '" size="13" readonly="true" class="form-control"/></div></div></td></tr>').appendTo(scntDiv);
    j++;
    return false;
}
...
{
    $('.sw-btn-finish').prop('disabled', false);
    // $(".sw-btn-finish").attr("disabled", false);
    ...
    $('#update_profile_image').on('click', function () {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data["error"] == true) {    
                        $('#alert_div').contents().clone().addClass('alert-danger').append(data['message']).prependTo('#panel_tab4_example11');
                    
                }
                else if (data['success'] ==true) {
                    $('#alert_div').contents().clone().addClass('alert-success').append(data['message']).prependTo('#panel_tab4_example11');
                    $('#profile_image').show();
