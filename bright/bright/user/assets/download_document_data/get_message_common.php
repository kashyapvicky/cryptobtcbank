...
if($_GET['msg']!="" and $_GET['msg']=="allu_active"){ ?>
    <table id="example">
        <thead>
         ...
         <tbody>
                <?php
                	$isql = "select `join`.id,`join`.username,`join`.name,`join`.email,`join`.phone,`join`.jdate,`join`.misc,`investment`.amount,`investment`.sdate from `join`,`investment` WHERE `join`.id = `investment`.mid"; 
				    $iresult = mysqli_query($s_dbid,$isql);
				     if(mysqli_num_rows($iresult)>0){
				       while(list($id,$username,$name,$email,$phone,$jdate,$status,$amount,$sdate) = mysqli_fetch_row($iresult)){	
				           ?>
                               <tr>
                                  <td><?=$id?></td>
                                  <td><?=$username?> $</td>
...
if($_GET['msg']!="" and $_GET['msg']=="allu"){ ?>
    <table id="example">
        <thead>
         ...
         <tbody>
                <?php
                	$isql = "select `id`,`username`,`name`,`phone`,`email`,'phone',`jdate`,`misc`,`password` from `join`"; 
				    $iresult = mysqli_query($s_dbid,$isql);
				     if(mysqli_num_rows($iresult)>0){
				       while(list($id,$username,$name,$phone,$email,$phone,$jdate,$status,$password) = mysqli_fetch_row($iresult)){	
				           ?>
                               <tr>
                                  <td><?=$id?></td>
                                  <td><?=$username?> $</td>
