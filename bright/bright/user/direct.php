...
function getusers($snode,$pos){
	global $s_dbid,$pid,$cnt,$sno,$records_ref;
		...
		$sql  = "SELECT id,name,username,email,phone,sponsor,DATE_FORMAT(jdate, '%d-%m-%Y'),position,misc,package,dreferal FROM `join` WHERE `dreferal` = '$snode' order by position";
        $result = mysqli_query($s_dbid,$sql);

	    if(mysqli_num_rows($result)>0){
			while(list($mid,$name,$username,$email,$phone,$sponsor,$jdate,$position,$status,$package,$direct) = @mysqli_fetch_row($result)){
				$ruser = $username;
				$cnt=1;
				$sno++;
				...
				echo "<tr><td align='center'>".$name."</td><td align='center'>".$username."</td><td align='center'>".$direct."</td><td align='center'>".$pos."</td><td align='center'>".$jdate."</td><td align='center'>".$sdate."</td><td align='center'>".$phone."</td><td align='center'> ".substr($amount,0,-6)."</td><td align='center'> ".$status."</td></tr>";
                    ...
                    <div class="col-md-12">
                      ...
                      <tbody>
                        ...
                        <?php 
							...
							$sql  = "SELECT id,name,username,email,phone,sponsor,DATE_FORMAT(jdate, '%d-%m-%Y'),position,dreferal FROM `join` WHERE `dreferal` = '$username' ";
        					$result = mysqli_query($s_dbid,$sql);

							while(list($mid,$name,$luser,$email,$phone,$sponsor,$jdate,$pos,$direct) = @mysqli_fetch_row($result)){
							
								$sql1 = "select amount,status,sdate from investment where mid = '$mid'";
								$result1 = mysqli_query($s_dbid,$sql1);
								list($amount,$status,$sdate) = mysqli_fetch_row($result1);
							
							echo "<tr><td align='center'>".$ctr."</td><td align='center'>".$name."</td><td align='center'>".$luser."</td><td align='center'>".$direct."</td><td align='center'>".$pos."</td><td align='center'>".$jdate."</td><td align='center'>".$sdate."</td><td align='center'> ".$phone."</td><td align='center'> ".$amount."</td><td align='center'> ".$status."</td></tr>";
							$ctr++;
								$amount="";
								$status="";
							}
